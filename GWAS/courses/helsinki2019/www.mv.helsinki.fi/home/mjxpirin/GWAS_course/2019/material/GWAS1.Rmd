...
and that the data we handle will almost certainly 
...
#### Contents of this course (slides 20-21)
...
7. Genetic correlation and Mendelian randomization

8. Summary statistics and meta-analysis
...
is called allele 0. 
(There is no general rule that GWAS results are reproted as 
...
These frequencies are called **Hardy-Weinberg equilibrium** (HWE) genotype frequencies 
...
without *selection*, *migration*, or *genetic drift*, i.e., 
statistical fluctuations due to finite population size.
In practice, most variants in many human populations seem to approximately follow HWE.
...
**Question.** How is the concept of credible interval (Bayesian) different from confidence interval (frequentist)?
...
as P-value is quite large (0.5361).
...
**Example 1.2. QT-GWAS (slides 14-16)** [GWAS on body-mass index (BMI)](https://www.nature.com/articles/nature14177) by Locke et al. (2015)
...
**Example 1.3. Disease GWAS (slides 17-19)** [GWAS on migraine](https://www.nature.com/articles/ng.3598) by Gormley et al. (2016) 
...
and highlighted enrichment of signals near genes that are active in 
in vascular system. 
...
We see that phenotype varies with genotype in such a way that each additional 
 ...
 * $\beta$, the effect on the mean phenotype per each copy of allele 1. 

The error terms
are assumed to have an identical Normal distribution $N(0,\sigma^2)$ where
$\sigma^2$ is not known and will be estimated from the data. Let's do it in R.
...
* P-value under the null hypothesis that the parameter is 0 and errors are uncorrelated and have a Gaussian distribution $N(0,\sigma^2)$. 

Under assumptions of linear model, sampling distribution of t-statistic is $t$-distribution and hence q% confidence intervals are determined as $\widehat{\beta}\pm a\times \textrm{SE}$, where $a$ is the q/2% quantile of $t$-distribution with $n-2$ degrees of freedom. 
...
and use z-scores and Normal approximation by default.

Last paragraph in output tells about the full model fit. 
We can measure how much variation in $y$ is left unexplained by the model 
by computing **residual sum of squares** (RSS):
$$
RSS = \sum_i \left(y_i - \widehat{\mu} - x_{i} \widehat{\beta}\right)^2
$$
$R^2$ is proportion of variance explained by the linear model, 
...
For this simple linear model, a scatter plot and regression line is a good way to
...
genotype and phenotype where a copy of allele T decreases LDL levels by 0.44 units. 
This variant explains about
1.5% of the variation of LDL-cholesterol levels. We also see that individuals homozygous
...
$p_2/(1-p_2) = 3.0\cdot p_1/(1-p_1) = 3.0\cdot 0.5294/ (1-0.5294) = 3.3748$ and
$p_2 = 3.3748 / (1 + 3.3748) = 0.7714$. That is, 77%.
...
$$\widehat{\textrm{OR}}_1 = \frac{R_1 S_0}{R_0 S_1},\qquad \log\left(\widehat{\textrm{OR}}_1 \right) = 
\log\left(\frac{R_1 S_0}{R_0 S_1}\right) \qquad \textrm{ and } \qquad
...
It explains logarithm of the odds of the disease by
...
Let's generate 1000 cases and controls from these genotype frequencies
...
In this course, we are not concentrating on the commands or input file formats
...
view point. These skills are independent of the GWAS software.
